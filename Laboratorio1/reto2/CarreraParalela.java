...
public class CarreraParalela {
    ...
    public static List<Integer> resultadosCombinados(List<Integer> lista) {
        CarreraParalela obj = new CarreraParalela();
        List<Integer> nuevaLista = new ArrayList<>();
        int maximo = obj.numeroMasGrande(lista);
        numPeqCant(lista); // Suma mínimo y cantidad
        nuevaLista.add(maximo);
        nuevaLista.add(obj.minimo);
        nuevaLista.add(obj.cantidad);
        return nuevaLista;
    }
    ...
    public List<Object> funcionFusionada(List<Integer> lista1, List<Integer> lista2) {
        ...
        int numeroMaximo = obj.numeroMasGrande(listaUnida);
        numPeqCant(listaUnida);


        boolean maxEsMultiplo = divDeDos(numeroMaximo);
        boolean cantEsPar = !datosImpares(listaUnida.size());

        List<Object> resultados = new ArrayList<>();
        resultados.add(numeroMaximo);
        resultados.add(minimo);
        resultados.add(listaUnida.size());
